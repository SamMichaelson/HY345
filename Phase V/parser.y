...
expr:           assiginexpr                 {$$=$1; }
                ...
                | expr AND expr             {$$=newexpr(boolexpr_e); $$->sym=newtemp(scope,yylineno); 
                                             emit(and,$1,$3,$$,nextquadlabel(),yylineno);
                                             }
                | expr OR expr              {$$=newexpr(boolexpr_e); $$->sym=newtemp(scope,yylineno); 
...
returnstmt:     RETURN{returned=1;} SEMICOLON{returned=0;        
                    ...
                    emit(jump,NULL,NULL,0,nextquadlabel()+1,yylineno);
                    $$.breakList=0; $$.contList=0; 
                }
...
int main(int argc , char** argv)
{
    ...
    for (i = 0; i < nextquadlabel(); i++) {
        ...
        if(instructions[i].result.val)
            data[i]=instructions[i].result.val;
        if(instructions[i].arg1.val)
            data[i]=instructions[i].arg1.val;
        if(instructions[i].arg2.val)
            data[i]=instructions[i].arg2.val;
    }
