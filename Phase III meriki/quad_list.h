...
SymbolTableEntry* newtemp(unsigned int scope,int line){
        ...
        if (sym==NULL){
            insertElemVariable(FORMAL,name,scope,line,currscopespace(),currscopeoffset());
...
unsigned makelist(unsigned label){
    return label;
}

void backpatch(unsigned label,unsigned i){
    quads[i].label=label;
    }

unsigned merge(unsigned E1, unsigned E2){
    if(!E1)
        return E2;
    else if(!E2)
        return E1;
      ...
      if (quads[i].result != NULL && quads[i].result->sym != NULL && quads[i].result->sym->value.varVal != NULL) { 
       ...
       if (quads[i].arg1 != NULL &&  quads[i].arg1->sym != NULL && quads[i].arg1->sym->value.varVal != NULL  ) {
        ...
        if (quads[i].arg2 != NULL && quads[i].arg2->sym != NULL && quads[i].arg2->sym->value.varVal != NULL) {
            ...
            printf("arg1: %5s,  ",quads[i].arg2->strConst);
