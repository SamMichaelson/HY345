        ...
        while (curr != NULL) {
            ...
            if (entry->value.funcVal != NULL && (entry->type==USERFUNC || entry->type==LIBFUNC)) {
               if(entry->type==USERFUNC){
                ...
                printf("Variable %s found in scope %d, line %d, space %s , offset %d\n", entry->value.varVal->name, entry->value.varVal->scope, entry->value.varVal->line, spaceString, entry->value.varVal->offset);
                reset();
            } 
            curr = curr->next;
